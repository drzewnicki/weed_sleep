...
MACS asks about marijuana use with a factor variable about average marijuana use since last visit ranging from not at all to daily. 
...
new_pot_use <- pot_use %>%
  ...
  group_by(MACSID) %>%
  mutate(avgpotuse = round(mean(HASHF)),
         mean_potuse = mean(HASHF)) %>%
  ungroup() %>%
  distinct(MACSID, .keep_all = TRUE) %>%
  dplyr::select(MACSID, avgpotuse, mean_potuse)

miss_var_summary(new_pot_use)

new_pot_use %>%
  group_by(avgpotuse) %>%
  count()

#includes 5, 7, 9, which are not possible values
new_pot_use <- new_pot_use %>%
  mutate(avgpotuse = ifelse(avgpotuse > 4, NA, avgpotuse))

new_pot_use <- new_pot_use %>%
  mutate(avgpotuse_f = factor(avgpotuse, ordered = TRUE, levels = c("0", "4", "3", "2", "1"), labels = c("Never", "Less Often", "Monthly", "Weekly", "Daily")),
         MACSID = as.character(MACSID)) 
...
myVars <- c("AGENOX", "race_collapsed", "SEROSTAT",  "schooling", "on_antidepressant", "cesd_factor",  "SLPWEED", "twopotuse", "slplat", "remlat", "remdur", "waso", "PSQIRATE" )

catVars <- c("race_collapsed", "SEROSTAT",  "schooling", "on_antidepressant", "cesd_factor")
...
```{r}
table_1_weed_strat <- CreateTableOne(vars = myVars, data = complete_cases, factorVars = catVars, strata = "SLPWEED")
table_1_weed_strat

# Convert to matrix, then data frame
tab_df <- as.data.frame(print(table_1_weed_strat, quote = FALSE, noSpaces = TRUE, printToggle = FALSE))

# Export to Word
ft <- flextable(cbind(rownames(tab_df), tab_df))
...
table_1_strat <- CreateTableOne(vars = myVars, data = complete_cases, factorVars = catVars, strata = "SEROSTAT")
table_1_strat
```
...
#merge in correct twopot

final_data <- final_data %>%
  left_join(new_pot_use) %>%
  dplyr::select(-c(twopotuse, twopotuse_n)) %>%
  rename(twopotuse = avgpotuse_f,
         twopotuse_n = mean_potuse)
...
rem_latency <- formatted_results(rem_latency)
rem_latency <- write.csv(rem_latency, file = "rem_latency.csv", row.names = FALSE)
...
rem_latency_results <- formatted_results(rem_latency)
...
sleep_latency_results <- write.csv(sleep_latency_results, file = "sleep_latency_results.csv", row.names = FALSE)
...
waso_results <-  write.csv(waso_results, file = "waso_results.csv", row.names = FALSE)
```
...
psqi <- MASS::polr(PSQIRATE ~ SLPWEED + race_collapsed + SEROSTAT + AGENOX + schooling + on_antidepressant + cesd_factor, data = complete_cases, Hess = TRUE)

results <- tidy(psqi, conf.int = TRUE, conf.level = 0.95, exponentiate = TRUE) |>
  mutate(p.value = 2 * pnorm(abs(statistic), lower.tail = FALSE))

# Check what types are present
results |> distinct(coef.type)

# Filter to predictors only
coefs <- results |> filter(coef.type == "coefficient")


ordinal_format <- function(outcome) { outcome %>%
  mutate(term = case_when(term == "SLPWEED1" ~"Used Cannabis Prior to Sleep",
                          term == "SLPWEED2" ~ "   Yes",
                          term == "race_collapsedWhite" ~ "Race",
                          term == "race_collapsedHispanic"~ "   Hispanic",
...
sleep_quality <- ordinal_format(coefs)

sleep_quality <- write.csv(sleep_quality, file = "sleep_quality.csv", row.names = FALSE)
...
final_model_twopot <- function(x, y) {
  model <- glm(x ~ twopotuse + race_collapsed + SEROSTAT + AGENOX + schooling + on_antidepressant + cesd_factor, data = y, family = "gaussian")
}

formatted_twopot <- function(outcome) { outcome %>%
  tidy_and_attach(conf.int = T) %>%
  tidy_add_reference_rows %>%
  dplyr::select(term, estimate, conf.low, conf.high, p.value)%>%
  filter(term != "(Intercept)")%>%
  mutate(term = case_when(term == "twopotuse.L" ~"   Less Often",
                          term == "twopotuse.Q" ~ "   Monthly",
                          term == "twopotuse.C" ~ "   Weekly",
                          term == "twopotuse^4" ~ "   Daily",
                          term == "race_collapsedWhite" ~ "Race",
                          term == "race_collapsedHispanic"~ "   Hispanic",
                          term == "race_collapsedBlack" ~ "   Black", 
                          term == "race_collapsedOther" ~ "   Other",
...
rem_lat_twopot <- formatted_twopot(rem_latency_twopot)

write.csv(rem_lat_twopot, file = "remlat_twopot.csv", row.names = FALSE)
...
waso_twopot_results <- formatted_twopot(waso_twopot)

write.csv(waso_twopot_results, file = "waso_twopot.csv", row.names = FALSE)
...
sleep_latency_twopot_results <- formatted_twopot(sleep_latency_twopot)

write.csv(sleep_latency_twopot_results, file = "sleep_latency_twopot.csv", row.names = FALSE)

```

```{r}

psqi_twopot <- MASS::polr(PSQIRATE ~ twopotuse + race_collapsed + SEROSTAT + AGENOX + schooling + on_antidepressant + cesd_factor, data = complete_cases, Hess = TRUE)

psqi_twopot <- tidy(psqi_twopot, conf.int = TRUE, exponentiate = TRUE) |>
  mutate(p.value = 2 * pnorm(abs(statistic), lower.tail = FALSE))

# Filter to predictors only
psqi_twopot <- psqi_twopot |> filter(coef.type == "coefficient")
```
...
fit4 <- with(imp_final, MASS::polr(PSQIRATE ~ SLPWEED + race_collapsed + SEROSTAT + AGENOX + schooling + on_antidepressant + CESD_high, Hess = TRUE))
...
weed_missing %>%
  ...
  filter(!is.na(EDUCA2)) %>%
  #772 
  filter(!is.na(twopotuse)) %>%
  #617 particpants
```
```{r}
newest_merged_data %>%
  dplyr::select(MACSID, VISIT, AGENOX, SLPWEED, SEROSTAT, CESD, twopotuse.y, remlat, PSQIRATE, tstmin, seslponset, SMOKE, slplat) %>%
  miss_var_summary()
```

```{r}
#make twopot function
twopot_cont <- function(x, y) {
  model <- glm(x ~ twopotuse_n + race_collapsed + SEROSTAT + AGENOX + schooling + on_antidepressant + cesd_factor, data = y, family = "gaussian")
}

rem_check <- twopot_cont(final_data$remlat, final_data)

rem_check <- formatted_results(rem_check)

write.csv(rem_check, file = "rem_check.csv", row.names = FALSE)


sleep_lat_check <- twopot_cont(final_data$slplat, final_data)
sleep_lat_check <- formatted_results(sleep_lat_check)
write.csv(sleep_lat_check, file = "sleep_lat_check.csv", row.names = FALSE)


waso_check <- twopot_cont(final_data$waso, final_data)
waso_check <- formatted_results(waso_check)
write.csv(waso_check, file = "waso_check.csv", row.names = FALSE)


```

```{r}
#sleep quality twopot_n

psqi_twopot_n <- MASS::polr(PSQIRATE ~ twopotuse_n + race_collapsed + SEROSTAT + AGENOX + schooling + on_antidepressant + cesd_factor, data = final_data, Hess = TRUE)

results <- tidy(psqi_twopot_n, conf.int = TRUE, conf.level = 0.95, exponentiate = TRUE) |>
  mutate(p.value = 2 * pnorm(abs(statistic), lower.tail = FALSE))
...
ordinal_format <- function(outcome) { outcome %>%
  mutate(term = case_when(term == "twopotuse_n" ~"Average Cannabis Use",
                          term == "race_collapsedWhite" ~ "Race",
                          term == "race_collapsedHispanic"~ "   Hispanic",
                          term == "race_collapsedBlack" ~ "   Black", 
                          term == "race_collapsedOther" ~ "   Other",
...
sleep_quality_n <- ordinal_format(coefs)

sleep_quality_n <- write.csv(sleep_quality_n, file = "sleep_quality_n.csv", row.names = FALSE)
```

```{r}
#save R data
saveRDS(final_data, file = "final_data.rds")
saveRDS(complete_cases, file = "complete_cases.rds")
