...
nearly_final_data <- almost_final_data %>%
  select(AGENOX, SLPWEED, race_collapsed, SEROSTAT, schooling, sleep_aid_collapsed, CESD, cesd_factor, twopotuse.y, remlat, PSQIRATE, tstmin, seslponset, SMOKE, slplat) %>%
  rename(twopotuse = twopotuse.y) %>%
  #WASO
  mutate(spt = ((tstmin*100)/seslponset)+0.5,
         waso = spt-tstmin)
...
final_model <- function(x) {
  model <- glm(x ~ factor(SLPWEED) + race_collapsed + factor(SEROSTAT) + AGENOX + schooling + relevel(sleep_aid_collapsed, ref = "Did not use") + cesd_factor, data = final_data, family = "gaussian")
